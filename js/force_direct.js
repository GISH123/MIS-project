...
function activate() {
	...
	d3.json("../data/20000.json", function (error, graph) {
		if (error) return;
...
function reloadForce() {
	...
	path = d3.select("#graphG")
		...
		.enter().append("path")
		.attr("class", "link")
		.style("marker-end", function (d) { return d.label === "分支機構 " ? "url(#branch)" : "url(#trade)"; });
...
function query(type) {
	...
	if(filtered_nodes.length > 0) {
		...
		for (var i=0; i<filtered_edges.length; i++) {
			...
			else {filtered_edges[i].attributes._linknum = 1;}
		}
