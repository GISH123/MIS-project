gexfD3
	= function() {
		...
		this.size = function(incSize) {
			...
			//Measure
			layoutSize = incSize;
			xExtent = d3.extent(nodes, function(d) {
				return parseFloat(d.x)
			})
			yExtent = d3.extent(nodes, function(d) {
		...
		this.dynamicAttribute = function(incAtt) {
			...
			if (nodeAttributes.indexOf(dAtt) > -1) {
				...
				nDE = d3.extent(nodes.filter(function(p) {
					return p.properties[dAtt] != 0
				}), function(d) {
					return parseInt(d.properties[dAtt])
				})
			}
			if (linkAttributes.indexOf(dAtt) > -1) {
				//          lDE = d3.extent(links, function(d) {return parseInt(d.properties[dAtt])})
				lDE = d3.extent(links.filter(function(p) {
					return p.properties[dAtt] != 0
				}), function(d) {
					return parseInt(d.properties[dAtt])
				})
